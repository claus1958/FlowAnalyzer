...
uses
  SysUtils, Classes, windows, winsock, StrUtils, Dialogs;
...
type
  TStr04 = array [0 .. 03] of AnsiChar;
  TStr07 = array [0 .. 06] of AnsiChar;
  TStr08 = array [0 .. 07] of AnsiChar;
  TStr12 = array [0 .. 11] of AnsiChar;
  TStr16 = array [0 .. 15] of AnsiChar;
  TStr20 = array [0 .. 19] of AnsiChar;
  TStr32 = array [0 .. 31] of AnsiChar;
  TStr36 = array [0 .. 35] of AnsiChar;
  TStr48 = array [0 .. 47] of AnsiChar;
  TStr52 = array [0 .. 51] of AnsiChar;
  TStr64 = array [0 .. 63] of AnsiChar;
  TStr72 = array [0 .. 71] of AnsiChar;


  TStr128 = array [0 .. 127] of AnsiChar;
  TStr208 = array [0 .. 207] of AnsiChar;
  TStr256 = array [0 .. 255] of AnsiChar;
  TStr512 = array [0 .. 511] of AnsiChar;

  TStr1024 = array [0 .. 1023] of AnsiChar;
...
type
  TDataBaseHeader = record
    ...
    copyright: TStr64; // copyright AnsiString
...
type
  ...
  TConBackup = record
    ...
    watch_password: array [0 .. 15] of AnsiChar; // slave server password
    watch_opposite: array [0 .. 23] of AnsiChar; // opposite server IP address and port
...
type
  ...
  TConGroupMargin = record
    ...
    swap_long, swap_short: double; // tickvalue for bid & ask
...
type
  ...
  TConHoliday = record
    ...
    month: integer; // month
    day: integer; // day
    from, fromto: integer; // work time-from & to (minutes)
...
type
  ...
  TConSymbol = record
    ...
    function getJSON(brokerId: integer): string;
    // beim Server ist brokerId die brokerId (es gibt 2 davon fьr LCG und AT)
  end;
...
type
  ...
  TFeedDescription = record
    ...
    copyright: TStr128; // copyright AnsiString
...
const
  ...
  CHART_RANGE_OUT = 1;
  // Data from the ranges "the beginning of history" - ChartInfo::from and ChartInfo::to - the end of history.
...
type
  ...
  TTickRecord = packed record
    ...
    flags: AnsiChar; // TICK_FLAG_* flags
  end;
...
type
  ...
  TUserRecord = record
    ...
    publickey: array [0 .. PUBLIC_KEY_SIZE - 1] of AnsiChar; // public key
    send_reports: integer; // send reports?
    balance_status: integer; // internal usage
    user_color: COLORREF; // user color
    unused: array [0 .. 39] of AnsiChar; // for future use
    api_data: array [0 .. 15] of AnsiChar; // for API usage
    function getJSON(accountId: integer): string; // inzwischen korrigiert (beim Server ist brokerId die accountId !)
  end;
...
type
  ...
  TGroupCommandInfo = packed record
    ...
    command: AnsiChar; // group coommand
...
type
  ...
  // vom FTCollector an Server binдr
  cwActionToServer = packed Record
    actionId: int64; // 8
    userId: integer; // 4
    accountId: integer; // 4
    typeId: integer; // 4
    ...
    // ACwAction = array [0 .. 0] of cwAction;// statisches Array. Hier sind Elemente auch ьber Pointer[i] ansprechbar. Aber nicht beim dynamischen Array
    DACwActionToServer = array of cwActionToServer;
    // PACwAction = ^ACwAction;

    PATradeRecord = ^TATradeRecord;
    TATradeRecord = array [0 .. 0] of TTradeRecord;
    DTATradeRecord = array of TTradeRecord; // ein dynamisches Array von TTradeRecord
  ...
  type
    ...
    PAMarginLevel = ^TAMarginLevel;     //spдter -> cwRatings
  ...
  type
    ...
    TRequestInfo = record
      ...
      status: AnsiChar; // request status
  ...
  type
    ...
    TMailBox = record
      ...
      body: PAnsiChar; // pointer to mail body
  ...
  type
    ...
    TNewsTopic = record
      ...
      body: PAnsiChar; // body (if present)
  ...
  type
    IManagerInterface = interface
      ...
      function ErrorDescription(errcode: integer): PAnsiChar; stdcall;
      procedure WorkingDirectory(path: PAnsiChar); stdcall;

      // ---- connection
      function Connect(server: PAnsiChar): integer; stdcall;
      function Disconnect: integer; stdcall;
      function IsConnected: integer; stdcall;
      function login(login: integer; password: PAnsiChar): integer; stdcall;
      function LoginSecured(KeyPath: PAnsiChar): integer; stdcall;
      function KeysSend(KeyPath: PAnsiChar): integer; stdcall;
      function Ping: integer; stdcall;
      function PasswordChange(password: PAnsiChar; IsInvestor: integer): integer; stdcall;
      ...
      function CfgUpdatePlugin(const Config: TConPlugin; const ParUpd: TAPluginCfg; total: integer): integer; stdcall;
      // ----  delete
      ...
      function SrvFeederLog(name: PAnsiChar; len: integer): PAnsiChar; stdcall;
      // ---- chart bases
      function ChartRequestObsolete(const Chart: TChartInfo; timesign: ptime_t; out total: integer)
        : PARateInfoOld; stdcall;
      function ChartAddObsolete(symbol: PAnsiChar; const period: integer; const Rates: TARateInfoOld;
        var count: integer): integer; stdcall; // todo: check Count (was the pointer
      function ChartUpdateObsolete(symbol: PAnsiChar; const period: integer; const Rates: TARateInfoOld;
        var count: integer): integer; stdcall; // todo: look above
      function ChartDeleteObsolete(symbol: PAnsiChar; const period: integer; const Rates: TARateInfoOld;
        var count: integer): integer; stdcall;
      // ---- performance info
      ...
      function BackupRequestUsers(filename: PAnsiChar; Request: PAnsiChar; var total: integer): PAUserRecord; stdcall;
      function BackupRequestOrders(filename: PAnsiChar; Request: PAnsiChar; var total: integer): PATradeRecord; stdcall;
      ...
      function AdmUsersRequest(group: PAnsiChar; out total: integer): PAUserRecord; stdcall;
      function AdmTradesRequest(group: PAnsiChar; OpenOnly: integer; out total: integer): PATradeRecord; stdcall;
      ...
      function SymbolGet(symbol: PAnsiChar; out CS: TConSymbol): integer; stdcall;
      function SymbolInfoGet(symbol: PAnsiChar; out SI: TSymbolInfo): integer; stdcall;
      function SymbolAdd(symbol: PAnsiChar): integer; stdcall;
      function SymbolHide(symbol: PAnsiChar): integer; stdcall;
      function SymbolChange(const Prop: TSymbolProperties): integer; stdcall;
      function SymbolSendTick(symbol: PAnsiChar; const bid, ask: double): integer; stdcall;
      // ---- manager commands
      ...
      // ---- journal
      function JournalRequest(mode: integer; from, Upto: time_t; filter: PAnsiChar; out total: integer)
        : PAServerLog; stdcall;
      // ---- databases: direct request to the server
      function UsersRequest(out total: integer): PAUserRecord; stdcall; // verwendet direkt
      ...
      function UsersGroupOp(const info: TGroupCommandInfo; const Logins: TIntArray): integer; stdcall;
      function UserPasswordCheck(login: integer; password: PAnsiChar): integer; stdcall;
      function UserPasswordSet(login: integer; password: PAnsiChar; IsInvestor, ClearPubkey: integer): integer; stdcall;
      ...
      function TradesUserHistory(login: integer; from, Upto: time_t; out total: integer): PATradeRecord; stdcall;
      function TradeCheckStops(const trade: TTradeTransInfo; price: double): integer; stdcall;
      // ---- reports
      function ReportsRequest(const Request: TReportGroupRequest; const Logins: TIntArray; total: pinteger)
        : PATradeRecord; stdcall;
      function DailyReportsRequest(const Request: TDailyGroupRequest; const Logins: TIntArray; total: pinteger)
        : PADailyReport; stdcall;
      // ---- external command
      function ExternalCommand(Data_in: PAnsiChar; Size_in: integer; Data_out: pPChar; Size_out: pinteger)
        : integer; stdcall;
      // ---- plugins
      function PluginUpdate(const plugin: TConPluginParam): integer; stdcall;
      // ---- pumping
      function PumpingSwitch(Callback: MTAPI_NOTIFY_FUNC; TargetW: HWND; EventMsg: uint; flags: integer)
        : integer; stdcall;
      function GroupsGet(out total: integer): PAConGroup; stdcall;
      function GroupRecordGet(name: PAnsiChar; var group: TConGroup): integer; stdcall;
      ...
      function UserRecordGet(login: integer; var User: TUserRecord): integer; stdcall; // geht nicht
      function OnlineGet(out total: integer): PAOnlineRecord; stdcall;
      function OnlineRecordGet(login: integer; var User: TOnlineRecord): integer; stdcall; // geht nicht
      function TradesGet(out total: integer): PATradeRecord; stdcall;
      function TradesGetBySymbol(symbol: PAnsiChar; out total: integer): PATradeRecord; stdcall;
      function TradesGetByLogin(login: integer; group: PAnsiChar; out total: integer): PATradeRecord; stdcall;
      ...
      function MarginsGet(out total: integer): PAMarginLevel; stdcall;          //alle zusammen
      function MarginLevelGet(login: integer; group: PAnsiChar; var margin: TMarginLevel): integer; stdcall;
      ...
      function MailLast(path: PAnsiChar; Length: pinteger): integer; stdcall;
      ...
      function NewsBodyGet(key: integer): PAnsiChar; stdcall;
      // ---- dealing
      function DealerSwitch(Callback: MTAPI_NOTIFY_FUNC; const TargetW: HWND; EventMsg: uint): integer; stdcall;
      ...
      // ----
      function TickInfoLast(symbol: PAnsiChar; out total: integer): PATickInfo; stdcall;
      ...
      function SummaryGet(symbol: PAnsiChar; var info: TSymbolSummary): integer; stdcall;
      ...
      function SummaryCurrency(Cur: PAnsiChar; MaxChars: integer): integer; stdcall;
      function ExposureGet(out total: integer): PAExposureValue; stdcall;
      function ExposureValueGet(Cur: PAnsiChar; var info: TExposureValue): integer; stdcall;
      // ----
      function MarginLevelRequest(login: integer; var Level: TMarginLevel): integer; stdcall;
      function HistoryCorrect(symbol: PAnsiChar; out updated: integer): integer; stdcall;
      // ---- new chart bases
      function ChartRequest(const Chart: TChartInfo; timesign: ptime_t; out total: integer): PARateInfo; stdcall;
      // geдndert statt ptime_t time_t
      function ChartAdd(symbol: PAnsiChar; const period: integer; const Rates: TARateInfo; var count: integer)
        : integer; stdcall;
      function ChartUpdate(symbol: PAnsiChar; const period: integer; const Rates: TARateInfo; var count: integer)
        : integer; stdcall;
      function ChartDelete(symbol: PAnsiChar; const period: integer; const Rates: TARateInfo; var count: integer)
        : integer; stdcall;
      // ---- ticks base
  ...
  type
    ...
    TMTManGetInterface = function(version: integer; var Obj: IManagerInterface): integer; cdecl;
...
// freeze_level: integer; // order modification freeze level
// unused: array [0 .. 25] of integer;
// function getJSON(brokerId: integer): string;   //beim Server ist brokerId die brokerId (es gibt 2 davon fьr LCG und AT)

function TConSymbol.getJSON(brokerId: integer): string;
begin
  result := '{' + c34 + 'brokerId' + c34 + ':' + inttostr(brokerId) + ',' + c34 + 'type' + c34 + ':' + inttostr(gtype) +
    ',' + c34 + 'name' + c34 + ':' + c34 + symbol + c34 + ',' + c34 + 'description' + c34 + ':' + c34 + description +
    c34 + ',' + c34 + 'currency' + c34 + ':' + c34 + currency + c34 + ',' + c34 + 'marginCurrency' + c34 + ':' + c34 +
    margin_currency + c34 + ',' + c34 + 'tradeMode' + c34 + ':' + inttostr(trade) + ',' + c34 + 'expiration' + c34 + ':'
    + inttostr(expiration) + ',' + c34 + 'contractSize' + c34 + ':' + FloatToSQLStr(contract_size) + ',' + c34 +
    'tickValue' + c34 + ':' + FloatToSQLStr(tick_value) + ',' + c34 + 'tickSize' + c34 + ':' +
    FloatToSQLStr(tick_size) + '}';
end;
...
function TTradeRecord.getJSON(accountId: integer): string;

begin
  // die accountId (ex Brokerid) wird  mit der order kombiniert zu einer eindeutigen ActionId
  result := '{' + c34 + 'accountId' + c34 + ':' + inttostr(accountId) + ',' + c34 + 'actionId' + c34 + ':' +
    inttostr(order) + inttostr(accountId) + ',' + c34 + 'userId' + c34 + ':' + inttostr(login) + ',' + c34 + 'symbol' +
    c34 + ':' + c34 + symbol + c34 + ',' + c34 + 'precision' + c34 + ':' + inttostr(digits) + ',' + c34 + 'typeId' + c34
    + ':' + inttostr(cmd + 1) + ',' + c34 + 'volume' + c34 + ':' + inttostr(volume) + ',' + c34 + 'openTime' + c34 + ':'
    + inttostr(open_time) + ',' + c34 + 'openPrice' + c34 + ':' + FloatToSQLStr(open_price) + ',' + c34 + 'stopLoss' +
    c34 + ':' + FloatToSQLStr(sl) + ',' + c34 + 'takeProfit' + c34 + ':' + FloatToSQLStr(tp) + ',' + c34 + 'closeTime' +
    c34 + ':' + inttostr(close_time) + ',' + c34 + 'expirationTime' + c34 + ':' + inttostr(expiration) + ',' + c34 +
    'sourceId' + c34 + ':' + inttostr((conv_reserv and 15) + 1) + ',' + c34 + 'swap' + c34 + ':' +
    FloatToSQLStr(storage) + ',' + c34 + 'closePrice' + c34 + ':' + FloatToSQLStr(close_price) + ',' + c34 + 'profit' +
    c34 + ':' + FloatToSQLStr(profit) + ',' + c34 + 'comment' + c34 + ':' + c34 + removeDoubleQuotes(comment) + c34 +
    ',' + c34 + 'conversionRate0' + c34 + ':' + FloatToSQLStr(conv_rates[0]) + ',' + c34 + 'conversionRate1' + c34 + ':'
    + FloatToSQLStr(conv_rates[1]) + ',' + c34 + 'marginRate' + c34 + ':' + FloatToSQLStr(margin_rate) + '}'
...
function TUserRecord.getJSON(accountId: integer): string;
begin
  result := '{' + c34 + 'accountId' + c34 + ':' + inttostr(accountId) + ',' + c34 + 'userId' + c34 + ':' +
    inttostr(login) + ',' + c34 + 'group' + c34 + ':' + c34 + group + c34 + ',' + c34 + 'name' + c34 + ':' + c34 + name
    + c34 + ',' + c34 + 'country' + c34 + ':' + c34 + country + c34 + ',' + c34 + 'city' + c34 + ':' + c34 + city + c34
    + ',' + c34 + 'state' + c34 + ':' + c34 + state + c34 + ',' + c34 + 'zipcode' + c34 + ':' + c34 + zipcode + c34 +
    ',' + c34 + 'address' + c34 + ':' + c34 + address + c34 + ',' + c34 + 'phone' + c34 + ':' + c34 + phone + c34 + ','
    + c34 + 'email' + c34 + ':' + c34 + email + c34 + ',' + c34 + 'comment' + c34 + ':' + c34 +
    removeDoubleQuotes(comment) + c34 + ',' + c34 + 'socialNumber' + c34 + ':' + c34 + id + c34 + ',' + c34 +
    'registrationTime' + c34 + ':' + inttostr(regdate) + ',' + c34 + 'lastLoginTime' + c34 + ':' + inttostr(lastdate) +
    ',' + c34 + 'leverage' + c34 + ':' + inttostr(leverage) + ',' + c34 + 'balancePreviousMonth' + c34 + ':' +
    FloatToSQLStr(prevmonthbalance) + ',' + c34 + 'balance' + c34 + ':' + FloatToSQLStr(balance) + ',' + c34 +
    'balanceLastDay' + c34 + ':' + FloatToSQLStr(prevbalance) + '}';

end;

constructor TCustomManager.Create(dll: String);
var
  GetVersion: TMTManGetVersion;
  GetInterface: TMTManGetInterface;
begin
  inherited Create;

  FLib := LoadLibrary(PWideChar(dll));
  Win32Check(FLib <> 0);

  @GetVersion := GetProcAddress(FLib, PAnsiChar(FNC_GETVERSN));
  @GetInterface := GetProcAddress(FLib, PAnsiChar(FNC_GETINTFC));
...
begin
  ...
  @FGetVersion := GetProcAddress(FLib, PAnsiChar(FNC_GETVERSN));
  @FGetInterface := GetProcAddress(FLib, PAnsiChar(FNC_GETINTFC));
end;
...
begin
  ...
  u := '\' + chr(34); // chr(39);
  // s:=ReplaceStr(s,t,'!');
  result := ReplaceStr(s, T, u);
end;

{ TMarginLevel }

function TMarginLevel.getJSON: string;
begin
  result := '{'
  + c34 + 'userId' + c34 + ':' + inttostr(login) + ','
  + c34 + 'balance' + c34 + ':' + FloatToSQLStr(balance) + ','
  + c34 + 'equity' + c34 + ':' + FloatToSQLStr(equity) + ','
  + c34 + 'volume' + c34 + ':' + inttostr(volume) + ','
  + c34 + 'margin' + c34 + ':' + FloatToSQLStr(margin) + '}';
end;
